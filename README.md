...
- **Model support for search.** `search_web` uses Gemini's Google Search
  grounding tool, which requires a model that supports it (the three in the
  dropdown do). If Google changes model names/availability, update the
  `<select id="model">` options in `public/index.html`.
